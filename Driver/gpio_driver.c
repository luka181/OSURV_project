...
#define GPIO_16 (16)
#define GPIO_17 (17) 
#define GPIO_18 (18) //triger
#define GPIO_19 (19)
#define GPIO_20 (20)
#define GPIO_21 (21)
#define GPIO_22 (22)
#define GPIO_23 (23)
#define GPIO_24 (24) //echo
#define GPIO_25 (25)
#define GPIO_26 (26)
#define GPIO_27 (27)
...
static ktime_t time_stop;
unsigned int time_passed;
long dist1, dist2, pom;
int trig;
int counter;
...
int gpio_driver_init(void)
{
	...
	//Trigger
	SetGpioPinDirection(GPIO_18, GPIO_DIRECTION_OUT);
...
{
	memset(gpio_driver_buffer, 0, BUF_LEN);
	sprintf(gpio_driver_buffer, "%u", time_passed);

	if (copy_to_user(buf, gpio_driver_buffer, len) != 0)
	{
		return -EFAULT;
	}
	else
...
{
	/* Reset memory. */
	printk(KERN_INFO "%c ali pre memseta\n", gpio_driver_buffer[0]);
	memset(gpio_driver_buffer, 0, BUF_LEN);


	/* Get data from user space.*/
	printk(KERN_INFO "poziv fje write\n");
	...
	else
	{
		...
		{
			printk(KERN_INFO "if\n");
			printk(KERN_INFO "%c", gpio_driver_buffer[0]);
			SetGpioPin(GPIO_18);
			udelay(12);
			ClearGpioPin(GPIO_18);


			while (GetGpioPinValue(GPIO_24) == 0)
			{

			}
			time_start = ktime_get(); //pocetak merenja echa senzora

			while (GetGpioPinValue(GPIO_24) == 1)
			{

			}
			time_stop = ktime_get();  //kraj merenja echa 

			time_passed = ktime_to_us(ktime_sub(time_stop, time_start));  //racunanje vremena trajanja echa senzora


		}
		else if ((gpio_driver_buffer[0] != '1') || (gpio_driver_buffer[0] != '0'))
			printk(KERN_INFO "Greska !!!");

		printk(KERN_DEBUG "Vreme: %u\n", time_passed);
